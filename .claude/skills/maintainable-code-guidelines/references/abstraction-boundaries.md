...
## Content-Pipeline / UI Split

The content layer is MDX compiled at build time by `@next/mdx`; there is no runtime database. When a component reaches around that pipeline — parsing markdown ad hoc, reading content files with `fs` at request time — content handling scatters across call sites instead of living in the one sanctioned pipeline.

**Guidelines:**

- MUST flag a component or request handler that reads MDX/content files from the filesystem or parses markdown itself instead of going through the `@next/mdx` pipeline (`page.mdx` routes and the shared `mdx-components.tsx` mapping).
- MUST flag a new module that derives data *about* content (e.g., a page index, titles, or navigation entries) with its own ad-hoc parsing when a shared helper for that derivation already exists — the derivation belongs in one shared module under `src/lib/`.
- MUST flag a helper under `src/lib/` that imports UI modules (components, routing, view libraries) — library helpers MUST be UI-free.
...
- MUST flag a client-side component that performs data fetching (network request, filesystem read, content derivation) — see the project's own component skill, if defined. Lift the fetch into the parent server-side component.
- MUST flag a client-side component that imports server-side content helpers or any module marked server-only. This will leak server code into the client bundle.
