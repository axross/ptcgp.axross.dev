...
|---|---|
| Page/view files (`page.mdx`, `page.tsx`, layouts) and the components they render | Rendered pages/views |
...
| MDX pipeline (`next.config.ts` MDX options, `src/mdx-components.tsx`, remark/rehype plugins) | Content fidelity on every page |
| Sentry config and instrumentation files | Observability |
...
- MUST verify the not-found / error state renders when the change affects routing or error handling (e.g., navigate to a non-existent URL).
...
This project relies on end-to-end tests as the primary verification mechanism. E2E tests run against the real application and cover the full rendering pipeline from MDX compilation through to the rendered output.
