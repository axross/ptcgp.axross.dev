...
- MUST begin every agent comment with the project's **one** fixed HTML marker line — `<!-- agent -->` — recorded here in this skill and reused identically across every run and session.
...
- MUST NOT push to the default branch (`main`); work on the harness's push-allowed branch prefix — `claude/`-prefixed branches in this project.
