...
- MUST require `npm run test:e2e` evidence after the change affects user-facing output, e2e coverage, snapshots, route behavior, metadata, content rendering, or other observable runtime state.
- MUST require `npm run build` evidence after the change affects routes, metadata, the MDX pipeline, runtime config, dependencies, or type signatures.
...
- SHOULD state each command with its observed result, such as "`npm run lint` passed" or "`npm run test:e2e` failed in a record-detail snapshot".
