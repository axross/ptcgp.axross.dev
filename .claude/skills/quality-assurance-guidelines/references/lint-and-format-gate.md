...
- MUST flag a new Biome's inline suppression (with justification) directive that lacks an inline justification on the same line. The project rule is "explain why, not just what".
- MUST flag a new block-level Biome suppression in any file that is not one of the whitelisted exception points the linter config defines.
